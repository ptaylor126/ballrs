...
const MAX_GUESSES = 8;
const STORAGE_KEY = 'ballrs_nfl_daily_game';
const STATS_KEY = 'ballrs_nfl_stats';
...
const STREAK_KEY = 'ballrs_nfl_daily_streak';
...
function getDailyPlayer(players: NFLPlayer[]): NFLPlayer {
  const dateString = getTodayString();
  // Use a different hash offset so NFL, NBA, and PL have different daily players
  let hash = 54321;
...
function comparePlayer(guessed: NFLPlayer, target: NFLPlayer): NFLMatchResult {
  ...
  const getPositionMatch = (): MatchStatus => {
    ...
    if (positionGroups[guessed.position] === positionGroups[target.position]) return 'partial';
    return 'wrong';
  };

  const getConferenceMatch = (): MatchStatus => {
    return guessed.conference === target.conference ? 'correct' : 'wrong';
  };

  const getDivisionMatch = (): MatchStatus => {
    if (guessed.division === target.division) return 'correct';
    if (guessed.conference === target.conference) return 'partial';
    return 'wrong';
  };

  return {
    team: guessed.team === target.team ? 'correct' : 'wrong',
    conference: getConferenceMatch(),
    division: getDivisionMatch(),
...
export default function NFLPuzzleScreen({ onBack }: Props) {
  const { user } = useAuth();
  const [players] = useState<NFLPlayer[]>((playersData as NFLPlayersData).players);
  const [dailyPlayer, setDailyPlayer] = useState<NFLPlayer | null>(null);
  const [guess, setGuess] = useState('');
  const [guessedPlayers, setGuessedPlayers] = useState<GuessedNFLPlayer[]>([]);
  ...
  async function loadGameState() {
    try {
      ...
      if (stored) {
        ...
        if (state.date === today) {
          const restoredGuesses: GuessedNFLPlayer[] = state.guessedPlayerIds
            .map(id => {
            ...
            .filter((g): g is GuessedNFLPlayer => g !== null);
  ...
  async function saveGameState(newGuessedPlayers: GuessedNFLPlayer[], newSolved: boolean) {
    try {
    ...
    if (stats.lastPlayedDate === today) {
      ...
      if (user) {
        ...
        if (currentCloudStats && won) {
          ...
          const updatedStats = await updateStatsAfterWin(user.id, 'nfl', currentCloudStats, dailyStreak);
          if (updatedStats) {
    ...
    if (user) {
      ...
      if (currentCloudStats) {
        if (won) {
          ...
          const updatedStats = await updateStatsAfterLoss(user.id, 'nfl');
          if (updatedStats) {
            setCloudStats(updatedStats);
          }
        }
  ...
  function handleSelectPlayer(selectedPlayer: NFLPlayer) {
    ...
    if (isCorrect) {
      ...
      if (guessCount >= MAX_GUESSES) {
        ...
        if (user) {
          awardLeaguePoints(user.id, 'nfl', guessCount, false);
        }
      }
    }
  }
  ...
  const displayStats = {
    currentStreak: cloudStats?.nfl_current_streak ?? stats.currentStreak,
    bestStreak: cloudStats?.nfl_best_streak ?? stats.bestStreak,
    totalSolved: cloudStats?.nfl_total_solved ?? stats.gamesWon,
  ...
  function getShareText() {
    ...
    return `Ballrs NFL 🏈 ${guessedPlayers.length}/${MAX_GUESSES}
${rows}
Streak: ${displayStats.currentStreak}`;
  ...
  function renderGuessRow(guessedPlayer: GuessedNFLPlayer, index: number) {
    ...
    return (
      <View key={player.id} style={styles.guessRow}>
        ...
        <View style={[styles.cell, getCellStyle(matches.conference)]}>
          <Text style={styles.cellText}>{player.conference}</Text>
        </View>
